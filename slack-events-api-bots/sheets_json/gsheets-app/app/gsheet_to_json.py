...
def open_google_spreadsheet(spreadsheet_url):
    '''
    ...
    spreadsheet = google_api_conn.open_by_url(spreadsheet_url)
    
    return spreadsheet

def fetch_data(spreadsheet_url):
    '''
    Opens the spreadsheet identified by `GOOGLE_SPREADSHEET_KEY`,
    then returns a dict object with data from the first sheet
    it contains.
    '''
    spreadsheet = open_google_spreadsheet(spreadsheet_url)
...
def store_json(data, target_filename, store_locally=TEST_MODE):
...
def update_json_data(spreadsheet_url=GOOGLE_SHEET_CONFIG['GOOGLE_SPREADSHEET_URL'], target_filename=S3_STORAGE_CONFIG['S3_TARGET_FILENAME']):
    data = fetch_data(spreadsheet_url)
    #print 'Fetched the data ...'

    data = transform_data(data)
    #print 'Prepped the data ...'

    store_json(data, target_filename)
