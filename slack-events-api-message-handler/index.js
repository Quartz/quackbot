                        ...
                        if (!team.verified) {
                            ...
                            return processWithNLP(event)
                                .then(nlpResult => {
                                    ...
                                    // if there's complex slack message returned from Dialogflow fulfillment ...
                                    if (event.nlp.fulfillment.data.hasOwnProperty("slack") && Object.getOwnPropertyNames(event.nlp.fulfillment.data.slack).length !== 0) {
                                        // send that block back to user
                                        sendToSlack(event, event.nlp.fulfillment.data.slack);
